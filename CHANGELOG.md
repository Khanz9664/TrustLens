...
- **Native LightGBM & CatBoost Backends**: Added automatic backend detection and prediction resolution for LightGBM (LGBMClassifier, Booster) and CatBoost (CatBoostClassifier) models, including probability extraction, classification validation, and integration with the standard PredictionBundle pipeline.
