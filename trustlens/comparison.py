...
def compare(reports: List[TrustReport]) -> None:
    ...
    for rep in reports:
        ...
        print(f"{model_name:<16} Score: {rep.trust_score.score:3d}/100 | Verdict: {rep.trust_score.verdict}")
    print()
    ...
    if not viable_candidates:
        ...
        for rep in blocked_candidates:
            ...
            pen_items = sorted(rep.trust_score.penalties_applied.items(), key=lambda x: x[1], reverse=True)
    ...
    if len(reports) > 1:
        ...
        if best_penalties < runner_penalties:
            ...
            penalties_sorted = sorted(runner_up.trust_score.penalties_applied.items(), key=lambda x: x[1], reverse=True)
        ...
        if not runner_up.trust_score.is_blocked:
            ...
            print(f"    - Note: {runner_name} was filtered out due to active diagnostic blocks ({runner_up.trust_score.verdict}).")
